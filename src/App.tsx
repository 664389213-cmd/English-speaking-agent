...
// Initialize speech synthesis voices for Chrome
function initializeSpeechSynthesis() {
  const synth = window.speechSynthesis;
  if (synth) {
    // Preload voices for Chrome compatibility
...
export default function App() {
  ...
  useEffect(() => {
    // Setup Speech Recognition with better error handling
    try {
      // @ts-ignore
      ...
      recognitionRef.current.onerror = (event: any) => {
        ...
        // Common Chrome errors - try to recover
        if (event.error === 'network') {
          console.warn('Network error - this may be a Chrome/VPN issue');
  ...
  const speakText = (text: string) => {
    ...
    try {
      ...
      // Add error handling for Chrome and other browsers
      utterance.onerror = (event) => {
        console.error('Speech synthesis error:', event.error);
      };

      // Use setTimeout to ensure Chrome has loaded voices
      setTimeout(() => {
        try {
  ...
  <div className="flex flex-col">
    ...
    <label className="text-[9px] font-bold text-text-sub/60 uppercase tracking-tight">PEP English(2024) Grade 8, Book 2</label>
              ...
              {typeof window !== 'undefined' && window.navigator.userAgent.includes('Chrome') && (
                <div className="text-[11px] text-center text-warning mt-3 px-3 py-2 bg-warning/10 rounded-lg border border-warning/30">
                  <strong>Chrome tip:</strong> If voice doesn't work, try Edge or Safari, or disable VPN/proxy and refresh.
                </div>
              )}
            </div>
          </div>
    ...
    <div className="h-screen w-full bg-brand-bg text-text-main flex flex-col overflow-hidden font-sans">
      ...
      <main className="flex-1 grid grid-cols-1 md:grid-cols-[1fr_380px] overflow-hidden">
          ...
          <div className="bg-gradient-to-r from-blue-50 via-blue-100/50 to-indigo-50 backdrop-blur-md border-b-4 border-primary p-6 sticky top-0 z-[10] shadow-xl">
            <div className="max-w-4xl mx-auto">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {/* 模块1: Tips 小贴士 */}
                <div className="bg-white p-4 rounded-3xl border-2 border-primary shadow-md">
                ...
                <div className="bg-white p-4 rounded-3xl border-2 border-primary shadow-md">
                  ...
                  <div className="flex flex-wrap gap-2">
                    ...
                    {selectedLevel === 'L1' && currentFullSentences.map((s, idx) => (
                      <div key={idx} className="bg-blue-50 border-2 border-primary px-3 py-1.5 rounded-xl text-[12px] text-text-main font-bold shadow-sm">
                        "{s}"
                      </div>
                    ))}
                    ...
                    {selectedLevel === 'L2' && currentStarters.map((pt, idx) => (
                      <button
                        ...
                        className="bg-orange-50 border-2 border-warning px-3 py-1.5 rounded-xl text-[12px] text-warning-shade font-bold shadow-sm hover:bg-warning/10 transition-colors"
                      >
                        {pt}
                      </button>
                    ))}
                  ...
                  <div className={cn(
                    ...
                    {msg.role === 'ai' ? (
                       <div className="flex flex-col gap-2">
                         ...
                         {msg.textCn && (
                           <div className="text-[12px] text-text-sub italic border-t border-text-sub/20 pt-2 mt-2">
                             {msg.textCn}
                           </div>
                         )}
                       </div>
        ...
        <aside className="bg-white p-6 flex flex-col gap-6 overflow-y-auto border-l border-brand-border">
          {/* 学习目标/核心句式/必会词汇 */}
          <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl p-5 border-2 border-primary shadow-md space-y-4">
            {/* Goals */}
            <div>
              ...
              <div className="grid grid-cols-1 gap-1">
                {(selectedScene.targetAwarenessCn || selectedScene.targetAwareness).map((goal, idx) => (
                  <div key={idx} className="text-[10px] font-bold text-text-main flex items-center gap-1.5">
                    <div className="w-1 h-1 rounded-full bg-primary/60"/>{goal}
                  </div>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-1.5">
                {selectedScene.preTaskReview.words.map(w => (
                  <span key={w} className="text-[10px] px-2 py-1 bg-primary/10 text-primary rounded font-bold border border-primary/20">{w}</span>
                ))}
              </div>
            </div>
          </div>
